...
class Markdown(Element):
    ...
    def _parse_markdown(self, text: str) -> List[Union[str, Element]]:
        ...
        for line in lines:
            ...
            elif re.match(r'^!$begin:math:display$.*$end:math:display$$begin:math:text$.*$end:math:text$$', line) or re.match(r'^$begin:math:display$.*$end:math:display$$begin:math:text$.*$end:math:text$$', line):
                if buffer:
                    elements.extend(self._parse_paragraph(" ".join(buffer)))
                    buffer = []
    ...
    def _parse_table(self, lines: List[str]) -> Element:
        ...
        for align_indicator in alignments.split('|')[1:-1]:
            align.append(
                'left' if align_indicator.startswith(':') and align_indicator.endswith('-') else
                'right' if align_indicator.endswith(':') else
                'center' if align_indicator.startswith(':') and align_indicator.endswith(':') else
                'left'
            )
        table_elements = [Tr(*[Td(cell.strip(), attributes={'style': f'text-align: {align[idx]}'}) for idx, cell in enumerate(header.split('|')[1:-1])])]
        for row in rows:
            table_elements.append(Tr(*[Td(cell.strip()) for cell in row.split('|')[1:-1]]))
    ...
    def _parse_code_block(self, lines: List[str]) -> Element:
        ...
        return Code("\n".join(code_lines), attributes={"class": f"language-{language}"} if language else None)
