...
class Haru:
    ...
    def __init__(self, import_name: str, asgi: bool = False):
        ...
        self.static_routes: List[Tuple[str, str, Optional[List[str]]]] = (
            []
        )  # (path, dir, ignore)
    ...
    def add_static_router(
        ...
        if not os.path.isdir(dir):
            raise ValueError(
                f"The directory '{dir}' does not exist or is not a directory."
            )
    ...
    def _check_static_route_overlaps(self) -> None:
        ...
        for path, _, _ in self.static_routes:
            if path in path_set:
                raise StaticRouteOverlapError(
                    f"Static route path '{path}' is already registered."
                )
            path_set.add(path)
    ...
    def run(
        ...
        if self.asgi:
            raise RuntimeError(
                "ASGI mode is enabled. Use an ASGI server to run the app."
            )
    ...
    def wsgi_app(
        ...
        try:
            ...
            route, params, allowed_methods = self.router.match(
            ...
            for mw in reversed(middlewares):
                middleware_result = self._run_middleware_method_sync(
    ...
    async def _asgi_app(
        ...
        if scope["type"] == "http":
            try:
                            ...
                            await send(
                                {
                                    ...
                                    "headers": [
                                        (
                                            b"allow",
                                            ", ".join(sorted(allowed_methods)).encode(
                                                "latin1"
                                            ),
                                        )
                                    ],
                ...
                for mw in reversed(middlewares):
                    result = await self._maybe_async(
                        mw.after_request, request, response
                    )
    ...
    def asgi_app(self) -> Callable:
        ...
        if not self.asgi:
            raise RuntimeError(
                "ASGI mode is not enabled. Pass 'asgi=True' when creating the app."
            )
    ...
    def _run_middleware_method_sync(
        self, method: Callable[..., Awaitable[T] | T], *args, **kwargs
    ) -> T:
        """
    ...
    def _register_static_route(
        self, path_prefix: str, directory: str, ignore: Optional[List[str]]
    ) -> None:
        """
        ...
        self.router.add_route(
            f"{path_prefix}/<filepath:path>", static_file_handler, methods=["GET"]
        )
